...
- **Cut / copy / paste**: the clipboard only stores a row reference. Cut rows are dimmed; on paste a cut row is moved and a copied row is deep-cloned (name gets ` copy`), both re-sorted afterwards. Right-clicking a file row means "paste into that file's directory" (a root-level file pastes into the root).
