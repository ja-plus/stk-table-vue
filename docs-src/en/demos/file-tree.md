...
- **Order comes from sorting only** (like VSCode): each level's `children` is sorted by name with `localeCompare`, so rows inside one folder cannot be reordered by dragging. Creating, renaming, moving and pasting all re-sort, and a renamed row jumps to its new sorted position.
