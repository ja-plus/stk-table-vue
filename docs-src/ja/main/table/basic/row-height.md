...
::: tip
上記の行の高さの設定と `props.expandConfig.height` はいずれも動的に変更できます。変更後、コンポーネントは表示行数とスクロール高さを自動的に再計算するため、`initVirtualScroll` を手動で呼び出す必要はありません。

フッター行はスクロール領域の下部に固定表示（sticky）されるため、仮想スクロールの総高さには含まれません。
:::
