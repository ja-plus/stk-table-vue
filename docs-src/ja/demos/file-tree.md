...
- **切り取り / コピー / 貼り付け**：クリップボードは行参照のみを保持します。切り取り中の行はグレー表示になり、貼り付け時は切り取りなら移動、コピーなら深いコピー（名前に ` copy` 付加）で、いずれも「フォルダ優先 + 名前」でソートし直されます。ファイル行を右クリックして貼り付ける場合は、そのファイルがあるディレクトリへの貼り付けを意味します（最上位のファイルなら最上位へ）。
