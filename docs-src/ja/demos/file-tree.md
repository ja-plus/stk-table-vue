...
- **右クリックメニュー**：`row-menu` イベントで [ja-contextmenu](https://github.com/ja-plus/ja-contextmenu) を表示します。「新しいファイル / 新しいフォルダ」はフォルダ行にのみ表示され、「貼り付け」は常に表示されますが、フォルダ行かつクリップボードに内容があるときだけ押せます（ファイル行ではグレー表示）。コピー / 切り取り後は任意のフォルダ行で必ず見つけられます。
...
- **切り取り / コピー / 貼り付け**：クリップボードは行参照のみを保持します。切り取り中の行はグレー表示になり、貼り付け時は切り取りなら移動、コピーなら深いコピー（名前に ` copy` 付加）で、いずれもソートし直されます。
