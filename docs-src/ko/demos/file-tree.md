...
- **우클릭 메뉴**: `row-menu` 이벤트에서 [ja-contextmenu](https://github.com/ja-plus/ja-contextmenu)를 띄웁니다. 폴더 행에는 "새 파일 / 새 폴더"가 추가되고, 클립보드에 내용이 있을 때만 폴더 행에 "붙여넣기"가 나타납니다.
...
- **새 파일 / 새 폴더**: 폴더 끝에 추가하고 곧바로 행 내 이름 바꾸기로 들어갑니다. 확정 전까지는 자리 표시 행입니다.
- **드래그로 이동**: 내장 `dragRow` 핸들 열은 쓰지 않습니다. 이름 셀 전체에 `draggable`을 붙이고 셀에서 네이티브 드래그 이벤트(`dragstart` / `dragover` / `drop`)를 직접 다룹니다. 폴더 행에 드롭하면 해당 폴더로 이동하고, 파일 행에 드롭한 경우 포인터가 칸의 위 / 아래 절반에 있는지에 따라 바로 앞 / 뒤에 삽입하며, 낙점에는 하이라이트 또는 삽입선이 표시됩니다. 폴더를 자신의 자손에 드롭하는 것은 막히며, 편집 중인 행은 입력창의 문자 선택을 위해 임시로 `draggable`을 해제합니다.
- **잘라내기 / 복사 / 붙여넣기**: 클립보드는 행 참조만 보관합니다. 잘라낸 행은 회색으로 표시되며, 붙여넣기 시 잘라내기는 이동, 복사는 깊은 복사(이름에 ` copy` 추가)입니다.

두 번째 표는 같은 데이터로 다른 배치 방법을 보여줍니다. 내장 화살표와 가이드선을 유지하고(`stkFoldIcon` 슬롯 렌더링) 폴더 이름에만 레이블을 붙입니다. 렌더링 비교용이며 파일 관리 조작은 첫 번째 표를 기준으로 합니다.
