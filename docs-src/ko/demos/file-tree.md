...
- **위치는 정렬로만 결정됩니다**(VSCode 방식): 각 레벨의 `children`을 이름의 `localeCompare`로 정렬하므로, 같은 폴더 안에서는 드래그로 순서를 바꿀 수 없습니다. 생성 · 이름 바꾸기 · 이동 · 붙여넣기 후 모두 다시 정렬되며, 이름을 바꾸면 행이 새 정렬 위치로 이동합니다.
