...
* `props.columns` 를 `v-model` 수식어로 변경해야 하며, 열 너비가 수정되면 직접 `StkTableColumn['width']` 값이 변경됩니다.
...
이렇게 하면 `columns` 앞에 `v-model` 수식어를 추가할 필요 없이, 수동으로 `StkTableColumn['width']` 값만 업데이트하면 됩니다.
