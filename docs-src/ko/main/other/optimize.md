...
## tr 레이어화
* CSS에서 `transform:translateZ(0)`을 설정하여 각 tr 행을 레이어화합니다. 이것은 성능에 도움이 됩니다.
  - 합성 레이어 승격으로 인해 검은 배경과 빨간 텍스트의 텍스트 색상이 변경될 수 있습니다.
  - ~~以下のケースでこの機能を有効にしてみてください~~
    - ~~복잡한 `customCell` 컴포넌트가 많은 경우~~
    - ~~강조 애니메이션이 많은 경우~~

스크롤 지연 감소가 눈에 띄는지 확인하기 위해以下のCSSを追加してみてください（低パフォーマンスマシンでより顕著）.
...
## props.autoResize
* 감시 성능 비용을 제거하려면手動으로 `props.autoResize=false`를 설정하세요. 너비와 높이가 고정된 테이블에 적합합니다.

## props.smoothScroll
* 일부 브라우저 버전에는 기본 관성 스크롤이 있습니다. 스크롤이 너무 빠르면 白屏가 발생할 수 있습니다. 따라서 Chrome > 85에서는 기본값으로 비활성화되어 있으며 `onwheel`을 사용하여 스크롤을 프록시하고 白屏를防止합니다.
