...
- **右键菜单**：在 `row-menu` 事件里弹出 [ja-contextmenu](https://github.com/ja-plus/ja-contextmenu)。「新建文件 / 新建文件夹」只对文件夹行显示；「粘贴」常驻显示，但只有文件夹行且剪贴板非空时才可点，文件行上置灰——复制 / 剪切之后右键任意文件夹行都能看到它。
...
- **剪切 / 复制 / 粘贴**：剪贴板只记行引用。剪切的行置灰；粘贴时剪切为移动、复制为深拷贝（名字加 ` copy`），落盘后同样重排。
