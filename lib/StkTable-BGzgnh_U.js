...
function useVirtualScroll(props, tableContainerRef, trRef, dataSourceCopy, tableHeaderLast, tableHeaders, rowKeyGen, maxRowSpan, scrollbarOptions, isExperimentalScrollY) {
 ...
 /** fix： 滚动条不在顶部时，表格数据变少，导致滚动条位置有误 */
	...
	function updateVirtualScrollY(sTop = 0) {
		...
		if (autoRowHeight || hasExpandCol.value) {
			...
			if (startIndex === oldStartIndex && endIndex === oldEndIndex) {
				if (virtualScroll.value.viewportStartIndex !== startIndex) virtualScroll.value.viewportStartIndex = startIndex;
				return;
			}
		}
		let viewportStartIndex = startIndex;
		if (maxRowSpan.size) {
		...
		*/
		if (!optimizeVue2Scroll || sTop <= scrollTop || Math.abs(oldStartIndex - startIndex) >= pageSize) Object.assign(virtualScroll.value, {
			...
			virtualScroll.value.viewportStartIndex = viewportStartIndex;
	...
	*
	* Optimization rules:
	* - Consecutive placeholder cells before the last must-render cell are merged into one {__VT_PH__: N}
	* - Cells after the last must-render cell are omitted entirely (browser handles missing cells)
	* - Cells covered by a previous row's rowspan are skipped (no td needed)
	* - If no must-render cells found, fall back to original columns (e.g., stripe-adjusted rows)
	*/
	const aboveViewportColumnMap = computed(() => {
		const map = /* @__PURE__ */ new Map();
		if (!virtual_on.value) return map;
		const { startIndex, viewportStartIndex } = virtualScroll.value;
		if (startIndex >= viewportStartIndex) return map;
		const columns = virtualX_columnPart.value;
		const data = dataSourceCopy.value;
		/** Coverage grid: rowOffset -> Set of colIdx covered by rowspan from earlier rows */
		const coverage = /* @__PURE__ */ new Map();
		const aboveCount = viewportStartIndex - startIndex;
		for (let absRowIndex = startIndex; absRowIndex < viewportStartIndex; absRowIndex++) {
			const row = data[absRowIndex];
			if (!row) continue;
			const rowOffset = absRowIndex - startIndex;
			const rowCols = [];
			let placeholderColspan = 0;
			let hasMustRender = false;
			for (let colIdx = 0; colIdx < columns.length; colIdx++) {
				var _coverage$get, _col$mergeCells;
				const col = columns[colIdx];
				if (col.__VT_C_SP__) {
					if (placeholderColspan > 0) {
						rowCols.push({ __VT_PH__: placeholderColspan });
						placeholderColspan = 0;
					}
					rowCols.push(col);
					continue;
				}
				if ((_coverage$get = coverage.get(rowOffset)) === null || _coverage$get === void 0 ? void 0 : _coverage$get.has(colIdx)) continue;
				const mergeInfo = ((_col$mergeCells = col.mergeCells) === null || _col$mergeCells === void 0 ? void 0 : _col$mergeCells.call(col, {
					row,
					col,
				...
				if (rowspan > 1) for (let r = 1; r < rowspan; r++) {
					...
					for (let c = 0; c < colspan; c++) coverage.get(futureOffset).add(colIdx + c);
				}
				if (absRowIndex + rowspan > viewportStartIndex) {
					if (placeholderColspan > 0) {
						rowCols.push({ __VT_PH__: placeholderColspan });
						placeholderColspan = 0;
					}
					rowCols.push(col);
					hasMustRender = true;
				} else placeholderColspan += colspan;
				colIdx += colspan - 1;
			}
			if (!hasMustRender) map.set(rowKeyGen(row), columns);
			else map.set(rowKeyGen(row), rowCols);
		}
		return map;
	});
	/**
	* Check if a row (by relative index in virtual_dataSourcePart) is above the viewport.
	* Above-viewport rows are those in [startIndex, viewportStartIndex) after rowspan correction.
	*/
	function isAboveViewportRow(rowIndex) {
		if (!virtual_on.value) return false;
		const { startIndex, viewportStartIndex } = virtualScroll.value;
		const aboveCount = viewportStartIndex - startIndex;
		return aboveCount > 0 && rowIndex < aboveCount;
	}
	return [
		virtualScroll,
		virtualScrollX,
...
var StkTable_default = /* @__PURE__ */ defineComponent({
	...
	setup(__props, { expose: __expose, emit: __emit }) {
		...
		*/
		...
		const [virtualScroll, virtualScrollX, virtual_on, virtual_dataSourcePart, virtual_offsetBottom, virtualX_on, virtualX_offsetRight, tableHeaderHeight, initVirtualScroll, initVirtualScrollY, initVirtualScrollX, updateVirtualScrollY, updateVirtualScrollX, setAutoHeight, clearAllAutoHeight, clearColWidthCache, virtualX_tableHeaders, expandRowColspan, theadVirtualX, virtualX_columnPart, aboveViewportColumnMap, isAboveViewportRow] = useVirtualScroll(props, tableContainerRef, trRef, dataSourceCopy, tableHeaderLast, tableHeaders, rowKeyGen, maxRowSpan, scrollbarOptions, isExperimentalScrollY);
		...
		*/
		...
		/**
		* Get the column list for a body row. For above-viewport rows, returns a modified list
		* with placeholder entries (__VT_PH__) to reduce DOM nodes.
		*/
		function getBodyColumns(row, rowIndex) {
			if (row && isAboveViewportRow(rowIndex)) return aboveViewportColumnMap.value.get(rowKeyGen(row)) || virtualX_columnPart.value;
					...
					createElementVNode("tbody", {
						...
						(openBlock(true), createElementBlock(Fragment, null, renderList(unref(virtual_dataSourcePart), (row, rowIndex) => {
							return openBlock(), createElementBlock("tr", mergeProps({
								...
								(openBlock(true), createElementBlock(Fragment, null, renderList(getBodyColumns(row, rowIndex), (col, _colIdx) => {
									...
									}, null, 8, _hoisted_17)) : !shouldHideCell(row, col) ? (openBlock(), createElementBlock("td", mergeProps({
										key: 2,
										ref_for: true
									...
									}, toDisplayString((row && row[col.dataIndex]) != null ? row && row[col.dataIndex] : getEmptyCellText.value(col, row)), 9, _hoisted_18)) : col.type === "seq" ? (openBlock(), createElementBlock("div", _hoisted_19, toDisplayString((props.seqConfig.startIndex || 0) + getAbsoluteRowIndex(rowIndex) + 1), 1)) : col.type === "tree-node" ? (openBlock(), createBlock(TreeNodeCell_default, {
										key: 3,
										class: "table-cell-wrapper",
										tabindex: "-1",
										col,
									...
									}, null, 8, ["onDragstart"])) : col.type === "expand" ? (openBlock(), createBlock(TriangleIcon_default, { key: 1 })) : createCommentVNode("", true), row[col.dataIndex] != null ? (openBlock(), createElementBlock("span", _hoisted_21, toDisplayString(row[col.dataIndex]), 1)) : createCommentVNode("", true)], 8, _hoisted_20))], 16)) : createCommentVNode("", true)], 64);
								}), 128)),
