	...
	function updateVirtualScrollY(sTop = 0) {
		...
		if (autoRowHeight || hasExpandCol.value) {
			...
			if (startIndex === oldStartIndex && endIndex === oldEndIndex) {
				...
				if (virtualScroll.value.viewportEndIndex !== endIndex) virtualScroll.value.viewportEndIndex = endIndex;
				return;
			}
		}
...
var StkTable_default = /* @__PURE__ */ defineComponent({
	...
	setup(__props, { expose: __expose, emit: __emit }) {
		...
		*/
		...
		const [virtualScroll, virtualScrollX, virtual_on, virtual_dataSourcePart, virtual_offsetBottom, virtualX_on, virtualX_offsetRight, tableHeaderHeight, initVirtualScroll, initVirtualScrollY, initVirtualScrollX, updateVirtualScrollY, updateVirtualScrollX, setAutoHeight, clearAllAutoHeight, clearColWidthCache, virtualX_tableHeaders, expandRowColspan, theadVirtualX, virtualX_columnPart, virtualX_expandColSegments] = useVirtualScroll(props, tableContainerRef, trRef, dataSourceCopy, tableHeaderLast, tableHeaders, rowKeyGen, maxRowSpan, getMaxRowSpanValue, scrollbarOptions, isExperimentalScrollY, mergeCellsCache);
