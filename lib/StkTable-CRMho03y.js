...
function useMergeCells(rowActiveProp, tableHeaderLast, rowKeyGen, colKeyGen, virtual_dataSourcePart, virtualScroll) {
	/**
	...
	*/
	...
	/** mergeCells result cache to avoid duplicate mergeCells calls */
	let mergeCellsCache = /* @__PURE__ */ new Map();
	...
	*/
	...
	/**
	* Get cached mergeCells result.
	* Cache is keyed by absolute rowIndex + leaf colIndex so that above-viewport
	* placeholder calculation and normal cell rendering share the same result.
	*/
	function getMergeCellsResult(row, col, rowIndex, colIndex) {
		...
		let cached = mergeCellsCache.get(cacheKey);
		if (!cached) {
			...
			mergeCellsCache.set(cacheKey, cached);
		}
		return cached;
	}
	/**
	...
	*/
	function mergeCellsWrapper(row, col, rowIndex, colIndex) {
		if (!col.mergeCells) return;
		const { colspan, rowspan } = getMergeCellsResult(row, col, rowIndex, colIndex);
	...
	*
	* This computed reuses getMergeCellsResult so mergeCells callbacks are not called
	...
	const aboveViewportColumnMap = computed(() => {
		...
		for (let rowOffset = 0; rowOffset < aboveCount; rowOffset++) {
			...
			for (let colIdx = 0; colIdx < columns.length; colIdx++) {
				...
				const { rowspan, colspan } = getMergeCellsResult(row, col, absRowIndex, colIdx);
...
var StkTable_default = /* @__PURE__ */ defineComponent({
	...
	setup(__props, { expose: __expose, emit: __emit }) {
		...
		*/
		...
		const [hiddenCellMap, mergeCellsWrapper, hoverMergedCells, updateHoverMergedCells, activeMergedCells, updateActiveMergedCells, aboveViewportColumnMap] = useMergeCells(rowActiveProp, tableHeaderLast, rowKeyGen, colKeyGen, virtual_dataSourcePart, virtualScroll);
