 ...
 * license: MIT
 */
import { Fragment, computed, createBlock, createCommentVNode, createElementBlock, createElementVNode, createTextVNode, createVNode, customRef, defineComponent, mergeProps, nextTick, normalizeClass, normalizeStyle, onBeforeUnmount, onMounted, onUnmounted, openBlock, provide, ref, renderList, renderSlot, resolveDynamicComponent, shallowRef, toDisplayString, toRaw, toRef, triggerRef, unref, watch, withCtx, withKeys, withModifiers } from "vue";
...
function _sfc_render$1(_ctx, _cache) {
	return openBlock(), createElementBlock("span", _hoisted_1$5, [..._cache[0] || (_cache[0] = [createElementVNode("svg", {
...
var DragHandle_default = /*#__PURE__*/ _plugin_vue_export_helper_default(_sfc_main$1, [["render", _sfc_render$1]]);
//#endregion
...
function _sfc_render(_ctx, _cache) {
	return openBlock(), createElementBlock("svg", _hoisted_1$4, [..._cache[0] || (_cache[0] = [createElementVNode("polygon", {
		class: "arrow-up",
		fill: "#757699",
...
var SortIcon_default = /*#__PURE__*/ _plugin_vue_export_helper_default(_sfc_main, [["render", _sfc_render]]);
//#endregion
//#region src/StkTable/components/TreeFoldIcon.vue?vue&type=script&setup=true&lang.ts
var _hoisted_1$3 = ["aria-expanded"];
//#endregion
//#region src/StkTable/components/TreeFoldIcon.vue
var TreeFoldIcon_default = /* @__PURE__ */ defineComponent({
	__name: "TreeFoldIcon",
	props: {
	...
	setup(__props) {
		...
		*/
		...
		return (_ctx, _cache) => {
			return openBlock(), createElementBlock("span", mergeProps(foldAttr.value, {
				class: stateClass.value,
				"aria-expanded": state.value === "icon" ? String(Boolean(__props.expanded)) : void 0
			}), null, 16, _hoisted_1$3);
		};
	}
});
//#endregion
//#region src/StkTable/components/TreeIndent.vue?vue&type=script&setup=true&lang.ts
var _hoisted_1$2 = {
	key: 0,
	class: "stk-tree-guide"
};
//#endregion
//#region src/StkTable/components/TreeIndent.vue
var TreeIndent_default = /* @__PURE__ */ defineComponent({
	__name: "TreeIndent",
	props: {
		level: {},
	...
	setup(__props) {
		return (_ctx, _cache) => {
			return openBlock(), createElementBlock("span", {
				class: "stk-tree-indent",
				style: normalizeStyle({ "--stk-tree-indent-cells": __props.level })
			}, [__props.showGuide && __props.level ? (openBlock(), createElementBlock("span", _hoisted_1$2)) : createCommentVNode("", true)], 4);
		};
	}
});
//#endregion
	...
	setup(__props) {
		return (_ctx, _cache) => {
			return openBlock(), createElementBlock("div", {
				...
				createVNode(TreeIndent_default, {
					level: __props.level ?? 0,
					"show-guide": __props.showGuide
				}, null, 8, ["level", "show-guide"]),
				createVNode(TreeFoldIcon_default, {
					expandable: __props.expandable,
					loading: __props.treeLoading,
					expanded: __props.treeExpanded ?? Boolean(__props.expanded)
				}, null, 8, [
					"expandable",
					"loading",
					"expanded"
				]),
				createElementVNode("span", null, toDisplayString(__props.cellValue ?? __props.row[__props.col.dataIndex] ?? ""), 1)
			], 8, _hoisted_1$1);
		};
	}
});
...
var loadingPromiseMap = /* @__PURE__ */ new WeakMap();
function useTree(props, dataSourceCopy, rowKeyGen, emits, onDataSourceChange) {
	...
	*/
	function isExpandable(row) {
		if (row.children !== void 0) return true;
		if (!isLazy()) return false;
		const { hasChildField } = props.treeConfig;
		return Boolean(row[hasChildField || "hasChildren"]);
	}
	/**
	...
	*/
	function ensureChildrenLoaded(row, col) {
		var _row$children;
		if (!row) return Promise.resolve(false);
		const pending = loadingPromiseMap.get(row);
		if (pending) return pending;
		if (!(isLazy() && !((_row$children = row.children) === null || _row$children === void 0 ? void 0 : _row$children.length) && isExpandable(row) && !row.__T_LOADED__)) return Promise.resolve(true);
		const { loadMethod, onLoadError } = props.treeConfig;
		if (!loadMethod) return Promise.resolve(false);
		row.__T_LOADING__ = true;
		const p = Promise.resolve().then(() => loadMethod(row, col)).then((children) => {
			row.children = children || [];
			row.__T_LOADED__ = true;
			return true;
		}, (error) => {
			onLoadError === null || onLoadError === void 0 || onLoadError(error, row, col);
			return false;
		}).finally(() => {
	...
	const toggleQueueMap = /* @__PURE__ */ new WeakMap();
	/** click expended icon to toggle expand row */
	function toggleTreeNode(row, col) {
		...
		toggleQueueMap.set(row, queue.finally(() => {
			toggleQueueMap.delete(row);
		}));
		return queue;
	}
	/** 队列 worker：执行单次展开意图（必要时先加载，重复点击时跳过已生效的意图） */
	async function runToggleIntent(row, col, wantExpand) {
		if (!wantExpand) {
			if (row.__T_EXP__) privateSetTreeExpand(row, {
				expand: false,
				col,
				isClick: true
			});
		...
		if (row.__T_EXP__) return;
		if (await ensureChildrenLoaded(row, col) && !row.__T_EXP__) privateSetTreeExpand(row, {
			expand: true,
			col,
			isClick: true
		});
	...
	function isChildrenReady(row) {
		var _row$children2;
		return !isLazy() || !isExpandable(row) || Boolean((_row$children2 = row.children) === null || _row$children2 === void 0 ? void 0 : _row$children2.length) || Boolean(row.__T_LOADED__);
	}
	/**
	*
	...
	*/
	...
	function setTreeExpand(row, option) {
		if (option === null || option === void 0 ? void 0 : option.parents) {
			...
			if (isLazy()) return setTreeExpandParentsLazy(row, option);
	...
	*/
	async function setTreeExpandParentsLazy(row, option) {
		const rowKeyOrRow = Array.isArray(row) ? row[0] : row;
		...
		if (!path) return;
		if (expanded) {
			for (let i = 0; i < path.length - 1; i++) privateSetTreeExpand(path[i], {
				expand: true,
				isClick: false
			});
			const target = path[path.length - 1];
			if (await ensureChildrenLoaded(target, null)) privateSetTreeExpand(target, {
				expand: true,
				isClick: false
			});
		} else {
	...
	*/
	async function resolveLazyPath(targetKey) {
		const roots = dataSourceCopy.value.filter((it) => !it.__T_LV__);
	...
	*/
	async function walkLazyPath(parent, path, targetKey) {
		if (!await ensureChildrenLoaded(parent, null)) {
			console.warn("tree lazy load failed on ancestor.rowKey:", rowKeyGen(parent));
			return null;
		}
		for (const child of parent.children || []) if (rowKeyGen(child) === targetKey) return [...path, child];
	...
	*/
	async function reloadTreeNode(rowKeyOrRow) {
		...
		if (wasExpanded) privateSetTreeExpand(row, {
			expand: true,
			isClick: false
		});
		else onDataSourceChange();
	}
	return [
		toggleTreeNode,
		setTreeExpand,
	...
	setup(__props, { expose: __expose, emit: __emit }) {
		...
		*/
		...
		function getTRProps(row, index) {
			...
			if (row === null || row === void 0 ? void 0 : row.__T_LOADING__) classList.push("is-tree-loading");
		...
		function onCellClick(e) {
			...
			if (!col) return;
			if ((_e$target = e.target) === null || _e$target === void 0 ? void 0 : _e$target.closest("[data-stk-fold], .stk-fold-icon")) {
		...
		*/
		function onHeaderCellKeydown(e, col) {
			if (!col.sorter) return;
			onColumnSort(col);
		}
		/**
		...
		*/
		...
		return (_ctx, _cache) => {
			return openBlock(), createElementBlock("div", {
				...
				tabindex: unref(areaSelectionConfig).enabled || unref(virtual_on) ? 0 : void 0,
						...
						return openBlock(), createElementBlock("tr", {
							...
							(openBlock(true), createElementBlock(Fragment, null, renderList(row, (col, colIndex) => {
								return openBlock(), createElementBlock("th", mergeProps({ key: colKeyGen.value(col) }, { ref_for: true }, getTHProps(col), {
									...
									onKeydown: [withKeys(withModifiers((e) => onHeaderCellKeydown(e, col), ["prevent"]), ["enter"]), withKeys(withModifiers((e) => onHeaderCellKeydown(e, col), ["prevent"]), ["space"])],
						...
						(openBlock(true), createElementBlock(Fragment, null, renderList(bodyRenderItems.value, (item) => {
							return openBlock(), createElementBlock(Fragment, { key: item.key }, [item.type === "row" ? (openBlock(), createElementBlock("tr", mergeProps({
								...
								(openBlock(true), createElementBlock(Fragment, null, renderList(getBodyColumns(item.row, item.rowIndex), (col, _colIdx) => {
									var _props$treeConfig, _props$treeConfig3;
									return openBlock(), createElementBlock(Fragment, { key: col.__VT_C_SP__ ? `spacer-${_colIdx}` : col.__VT_PH__ ? `ph-${_colIdx}` : colKeyGen.value(col) }, [col.__VT_C_SP__ ? (openBlock(), createElementBlock("td", {
										...
										level: col.type === "tree-node" ? item.row.__T_LV__ || 0 : void 0,
										expandable: col.type === "tree-node" ? unref(isTreeExpandable)(item.row) : void 0,
										"tree-loading": col.type === "tree-node" ? Boolean(item.row.__T_LOADING__) : void 0,
										"show-guide": col.type === "tree-node" ? (_props$treeConfig = props.treeConfig) === null || _props$treeConfig === void 0 ? void 0 : _props$treeConfig.showGuide : void 0
									}, {
										stkFoldIcon: withCtx(() => [createVNode(TreeFoldIcon_default, {
											expandable: col.type === "expand" || unref(isTreeExpandable)(item.row),
										...
										stkTreeIndent: withCtx(() => {
											var _props$treeConfig2;
											return [createVNode(TreeIndent_default, {
												level: item.row.__T_LV__ || 0,
												"show-guide": (_props$treeConfig2 = props.treeConfig) === null || _props$treeConfig2 === void 0 ? void 0 : _props$treeConfig2.showGuide
											}, null, 8, ["level", "show-guide"])];
										}),
										...
										"col-index": col.__LF_S__ ?? 0,
										"tree-expanded": Boolean(item.row.__T_EXP__),
										"tree-loading": Boolean(item.row.__T_LOADING__),
										level: item.row.__T_LV__ || 0,
										expandable: unref(isTreeExpandable)(item.row),
										"show-guide": (_props$treeConfig3 = props.treeConfig) === null || _props$treeConfig3 === void 0 ? void 0 : _props$treeConfig3.showGuide
									}, null, 8, [
										"col",
										"row",
										"cell-value",
									...
									}, null, 8, ["onDragstart"])) : col.type === "expand" ? (openBlock(), createBlock(TreeFoldIcon_default, {
										key: 1,
										expandable: true,
										expanded: Boolean(item.row.__EXP__ && colKeyGen.value(item.row.__EXP__) === colKeyGen.value(col))
									}, null, 8, ["expanded"])) : createCommentVNode("", true), item.row[col.dataIndex] != null ? (openBlock(), createElementBlock("span", _hoisted_21, toDisplayString(item.row[col.dataIndex]), 1)) : createCommentVNode("", true)], 8, _hoisted_20))], 16)) : createCommentVNode("", true)], 64);
								}), 128)),
...
//#endregion
export { registerFeature as a, insertToOrderedArray as c, TreeFoldIcon_default as i, strCompare as l, TreeNodeCell_default as n, useAreaSelection as o, TreeIndent_default as r, binarySearch as s, StkTable_default as t, tableSort as u };
