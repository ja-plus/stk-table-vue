 ...
 * license: MIT
 */
import { child, computed, createComponent, createDynamicComponent, createFor, createIf, createSlot, createTemplateRefSetter, customRef, defineVaporComponent, extend, next, nextTick, on, onBeforeUnmount, onMounted, onUnmounted, provide, ref, renderEffect, setAttr, setClass, setClassName, setDynamicProps, setInsertionState, setProp, setStaticTemplateRef, setStyle, setText, shallowRef, template, toDisplayString, toRaw, toRef, triggerRef, txt, unref, watch } from "vue";
...
var _sfc_main$2 = { __vapor: true };
var t0$4 = template("<span class=drag-row-handle draggable=true><svg viewBox=\"0 0 1024 1024\" width=20 height=20 fill=currentColor><path d=\"M640 853.3a85.3 85.3 0 1 1 85.3-85.3 85.3 85.3 0 0 1-85.3 85.3z m-256 0a85.3 85.3 0 1 1 85.3-85.3 85.3 85.3 0 0 1-85.3 85.3z m256-256a85.3 85.3 0 1 1 85.3-85.3 85.3 85.3 0 0 1-85.3 85.3z m-256 0a85.3 85.3 0 1 1 85.3-85.3 85.3 85.3 0 0 1-85.3 85.3z m256-256a85.3 85.3 0 1 1 85.3-85.3 85.3 85.3 0 0 1-85.3 85.3zM384 341.3a85.3 85.3 0 1 1 85.3-85.3 85.3 85.3 0 0 1-85.3 85.3z\">", 3);
function _sfc_render$2(_ctx, $props, $emit, $attrs, $slots) {
	return t0$4();
}
...
var _sfc_main$1 = { __vapor: true };
var t0$3 = template("<svg xmlns=http://www.w3.org/2000/svg width=16px height=16px viewBox=\"0 0 16 16\"><polygon class=arrow-up fill=#757699 points=\"8 2 4.8 6 11.2 6\"></polygon><polygon class=arrow-down transform=\"translate(8, 12) rotate(-180) translate(-8, -12) \" points=\"8 10 4.8 14 11.2 14\">", 3, 1);
function _sfc_render$1(_ctx, $props, $emit, $attrs, $slots) {
	return t0$3();
}
...
var _sfc_main = { __vapor: true };
var t0$2 = template("<div class=stk-fold-icon>", 3);
...
//#endregion
//#region src/StkTable/components/TreeNodeCell.vue?vue&type=script&setup=true&vapor=true&lang.ts
var t0$1 = template("<div><!><span> ", 1);
//#endregion
//#region src/StkTable/components/TreeNodeCell.vue
var TreeNodeCell_default = /* @__PURE__ */ defineVaporComponent({
	__name: "TreeNodeCell",
	props: {
		col: {},
		row: {}
	...
	setup(__props, { emit: __emit }) {
		...
		const n3 = next(n4);
		renderEffect(() => {
			const _row = __props.row;
			const _col = __props.col;
			const _row_T_LV = _row.__T_LV__;
			setProp(n5, "title", _row[_col.dataIndex] || "");
			setStyle(n5, _row_T_LV ? `padding-left:${_row_T_LV * 16}px` : "");
		});
		setInsertionState(n5, n4);
		createIf(() => __props.row.children !== void 0, () => {
			const _on_click = () => emit("click");
			return createComponent(TriangleIcon_default, { onClick: () => _on_click });
		});
		const x3 = txt(n3);
		renderEffect(() => {
			const _row = __props.row;
			const _col = __props.col;
			setStyle(n3, !_row.children ? "padding-left: 16px;" : null);
			setText(x3, toDisplayString(_row[_col.dataIndex] ?? ""));
		});
		return n5;
	}
});
 ...
 /** fix： 滚动条不在顶部时，表格数据变少，导致滚动条位置有误 */
	...
	function initVirtualScrollX() {
		...
		assignVs(virtualScrollX, {
			containerWidth: clientWidth || 200,
	...
	*/
	...
	function updateVirtualScrollY(sTop = 0) {
		...
		*/
		if (!optimizeVue2Scroll || sTop <= scrollTop || Math.abs(oldStartIndex - startIndex) >= pageSize) assignVs(virtualScroll, {
			startIndex,
			endIndex,
			offsetTop,
			viewportStartIndex,
...
//#endregion
//#region src/StkTable/StkTable.vue?vue&type=script&setup=true&vapor=true&lang.ts
var t0 = template("<div class=row-by-row-table-height>");
var t1 = template("<div class=column-resize-indicator>");
var t2 = template("<col>");
var t3 = template("<colgroup>");
var t4 = template("<th class=vt-x-left>");
var t5 = template("<div class=\"table-header-resizer left\">");
var t6 = template("<span class=table-header-title> ");
var t7 = template("<div class=\"table-header-resizer right\">");
var t8 = template("<th><!><div class=table-header-cell-wrapper></div>");
var t9 = template("<th class=vt-x-right>");
var t10 = template("<tr>");
var t11 = template("<thead>");
var t12 = template("<td class=vt-x-left>");
var t13 = template("<td class=vt-x-spacer>");
var t14 = template("<span> ");
var t15 = template("<td><!><div class=table-cell-wrapper tabindex=-1>");
var t16 = template("<td class=vt-x-right>");
var t17 = template("<td>");
var t18 = template("<tr class=padding-top-tr>");
var t19 = template(" ");
var t20 = template("<td><div class=table-cell-wrapper tabindex=-1>");
var t21 = template("<td class=vt-above-viewport-ph>");
var t22 = template("<div class=table-cell-wrapper tabindex=-1> ");
var t23 = template("<div class=table-cell-wrapper tabindex=-1>");
var t24 = template("<tr class=vt-above-viewport-ph-row>");
var t25 = template("<tr class=vt-below-viewport-ph>");
var t26 = template("<div class=\"stk-sb-thumb vertical\">");
var t27 = template("暂无数据", 2);
var t28 = template("<div>");
var t29 = template("<div class=\"stk-sb-thumb horizontal\">");
var t30 = template("<div><!><!><div class=stk-table-scroll-container><table><!><!><!><tbody class=stk-tbody-main></table></div>", 1);
//#endregion
//#region src/StkTable/StkTable.vue
var StkTable_default = /* @__PURE__ */ defineVaporComponent({
	name: "StkTable",
	props: {
		width: { default: "" },
		minWidth: { default: "" },
	...
	setup(__props, { expose: __expose, emit: __emit }) {
		...
		*/
		...
		const n179 = t30();
		const n178 = child(n179);
		const n180 = next(n178);
		const n167 = next(n180);
		const n161 = child(n167);
		const n160 = child(n161);
		const n162 = next(n160);
		const n163 = next(n162);
		const n159 = next(n163);
		renderEffect(() => {
			const _theme = __props.theme;
			const _props = props;
			const _scrollbarOptions = scrollbarOptions.value;
			const _areaSelectionConfig = unref(areaSelectionConfig);
			const _highlightSteps = unref(highlightSteps);
			const _props_bordered = _props.bordered;
			const _props_autoRowHeight = _props.autoRowHeight;
			const _areaSelectionConfig_enabled = _areaSelectionConfig.enabled;
			const _props_highlightConfig = _props.highlightConfig;
			setClass(n179, ["stk-table", {
				...
				"col-resizable": _props.colResizable,
				bordered: _props_bordered,
				[`bordered-${_props_bordered}`]: typeof _props_bordered === "string",
				stripe: _props.stripe,
				"cell-hover": _props.cellHover,
				"cell-active": _props.cellActive,
				"row-hover": _props.rowHover,
				"row-active": rowActiveProp.value.enabled,
				"text-overflow": _props.showOverflow,
				"header-text-overflow": _props.showHeaderOverflow,
				"fixed-relative-mode": isRelativeMode.value,
				"auto-row-height": _props_autoRowHeight,
				"scroll-row-by-row": unref(isSRBRActive),
				"scrollbar-on": _scrollbarOptions.enabled,
				"area-selection": _areaSelectionConfig_enabled,
				"is-area-selecting": unref(isAreaSelecting),
				"exp-scroll-y": isExperimentalScrollY.value
			}]);
			setProp(n179, "tabindex", _areaSelectionConfig_enabled ? 0 : void 0);
			setStyle(n179, {
				"--row-height": _props_autoRowHeight ? void 0 : unref(virtualScroll).rowHeight + "px",
				"--header-row-height": _props.headerRowHeight + "px",
				"--footer-row-height": _props.footerRowHeight + "px",
				"--highlight-duration": _props_highlightConfig.duration && _props_highlightConfig.duration + "s",
				"--highlight-timing-function": _highlightSteps ? `steps(${_highlightSteps})` : void 0,
				"--sb-width": `${_scrollbarOptions.width}px`,
				"--sb-height": `${_scrollbarOptions.height}px`
			});
		});
		setInsertionState(n179, n178);
		createIf(() => !isExperimentalScrollY.value && SRBRTotalHeight.value, () => {
			const n2 = t0();
			renderEffect(() => setStyle(n2, `height: ${SRBRTotalHeight.value}px`));
			return n2;
		});
		setInsertionState(n179, n180);
		createIf(() => __props.colResizable, () => {
			const n5 = t1();
			setStaticTemplateRef(n5, colResizeIndicatorRef, null, "colResizeIndicatorRef");
			return n5;
		});
		renderEffect(() => {
			setClassName(n161, props.fixedMode ? 1 : 0, " fixed-mode", "stk-table-main");
			setStyle(n161, {
				width: __props.width,
		...
		setInsertionState(n161, n160);
		createIf(() => props.fixedMode && !unref(virtualX_on), () => {
			const n11 = t3();
			setInsertionState(n11);
			createFor(() => tableHeaderLast.value, (_for_item0) => {
				const n10 = t2();
				renderEffect(() => setStyle(n10, getColGroupColStyle(_for_item0.value)));
				return n10;
			}, (col) => colKeyGen.value(col), 9);
			return n11;
		});
		setInsertionState(n161, n162);
		createIf(() => !__props.headless, () => {
			const n44 = t11();
			setInsertionState(n44);
			createFor(() => unref(virtualX_on) ? unref(virtualX_tableHeaders) : unref(tableHeaders), (_for_item0, _for_key0) => {
				const n43 = t10();
				setInsertionState(n43);
				createIf(() => unref(virtualX_on), () => {
					const n18 = t4();
					renderEffect(() => {
						const _theadVirtualX_offsetLeft = unref(theadVirtualX).offsetLeft;
						setStyle(n18, `min-width:${_theadVirtualX_offsetLeft}px;width:${_theadVirtualX_offsetLeft}px`);
					});
					return n18;
				});
				setInsertionState(n43, 1);
				createFor(() => _for_item0.value, (_for_item1, _for_key1) => {
					const n39 = t8();
					const n38 = child(n39);
					const n34 = next(n38);
					renderEffect(() => setDynamicProps(n39, [getTHProps(_for_item1.value)]));
					setInsertionState(n39, n38);
					createIf(() => unref(colResizeOn)(_for_item1.value) && _for_key1.value > 0, () => {
						const n23 = t5();
						on(n23, "mousedown", ($event) => unref(onThResizeMouseDown)($event, _for_item1.value, true));
						return n23;
					});
					renderEffect(() => {
						const _col_R_SP = _for_item1.value.__R_SP__;
						setStyle(n34, _col_R_SP ? `--row-span:${_col_R_SP}` : null);
					});
					setInsertionState(n34);
					createIf(() => _for_item1.value.customHeaderCell, () => {
						return createDynamicComponent(() => _for_item1.value.customHeaderCell, {
							col: () => _for_item1.value,
							colIndex: () => _for_key1.value,
							rowIndex: () => _for_key0.value
						});
					}, () => {
						return createSlot("tableHeader", { col: () => _for_item1.value }, () => {
							const n30 = t6();
							const x30 = txt(n30);
							renderEffect(() => setText(x30, toDisplayString(_for_item1.value.title)));
							return n30;
						});
					}, 1541);
					setInsertionState(n34, 1);
					createIf(() => _for_item1.value.sorter, () => {
						return createComponent(SortIcon_default, { class: "table-header-sorter" });
					});
					setInsertionState(n39, 2);
					createIf(() => unref(colResizeOn)(_for_item1.value), () => {
						const n37 = t7();
						on(n37, "mousedown", ($event) => unref(onThResizeMouseDown)($event, _for_item1.value));
						return n37;
					});
					on(n39, "click", (e) => onHeaderCellClick(e, _for_item1.value));
					on(n39, "dragstart", (e) => unref(onThDragStart)(e));
					on(n39, "drop", (e) => unref(onThDrop)(e));
					on(n39, "dragover", (e) => unref(onThDragOver)(e));
					return n39;
				}, (col, colIndex) => colKeyGen.value(col), 8);
				setInsertionState(n43, 2);
				createIf(() => unref(virtualX_on), () => {
					const n42 = t9();
					renderEffect(() => {
						const _virtualX_offsetRight = unref(virtualX_offsetRight);
						setStyle(n42, `min-width:${_virtualX_offsetRight}px;width:${_virtualX_offsetRight}px`);
					});
					return n42;
				});
				on(n43, "contextmenu", ($event) => onHeaderMenu($event));
				return n43;
			}, (row, rowIndex) => rowIndex, 9);
			return n44;
		});
		setInsertionState(n161, n163);
		createIf(() => __props.footerData && __props.footerData.length > 0, () => {
			return createDynamicComponent(() => footerTagName.value, {
				class: "stk-footer",
				style: () => isFooterTop.value ? `top:${unref(tableHeaderHeight)}px` : ""
			}, extend(() => {
				return createFor(() => __props.footerData, (_for_item0, _for_key0) => {
					const n71 = t10();
					setInsertionState(n71);
					createIf(() => unref(virtualX_on), () => {
						const n51 = t12();
						renderEffect(() => {
							const _theadVirtualX_offsetLeft = unref(theadVirtualX).offsetLeft;
							setStyle(n51, `min-width:${_theadVirtualX_offsetLeft}px;width:${_theadVirtualX_offsetLeft}px`);
						});
						return n51;
					});
					setInsertionState(n71, 1);
					createFor(() => unref(virtualX_columnPart), (_for_item1, _for_key1) => {
						return createIf(() => _for_item1.value.__VT_C_SP__, () => {
							const n56 = t13();
							renderEffect(() => setProp(n56, "colspan", _for_item1.value.__VT_C_SP__));
							return n56;
						}, () => {
							const n66 = t15();
							const n65 = child(n66);
							const n64 = next(n65);
							renderEffect(() => setDynamicProps(n66, [getTFProps(_for_item1.value)]));
							setInsertionState(n66, n65);
							createIf(() => _for_item1.value.customFooterCell, () => {
								return createDynamicComponent(() => _for_item1.value.customFooterCell, {
									class: "table-cell-wrapper",
									tabindex: "-1",
									col: () => _for_item1.value,
									row: () => _for_item0.value,
									rowIndex: () => _for_key0.value,
									cellValue: () => _for_item0.value[_for_item1.value.dataIndex]
								});
							});
							renderEffect(() => {
								const _col = _for_item1.value;
								setProp(n64, "title", _for_item0.value[_col.dataIndex] || "");
							});
							setInsertionState(n64);
							createIf(() => _for_item0.value[_for_item1.value.dataIndex] != null, () => {
								const n63 = t14();
								const x63 = txt(n63);
								renderEffect(() => {
									const _col = _for_item1.value;
									setText(x63, toDisplayString(_for_item0.value[_col.dataIndex]));
								});
								return n63;
							});
							return n66;
						}, 3082);
					}, (col, _colIdx) => col.__VT_C_SP__ ? `spacer-${_colIdx}` : colKeyGen.value(col), 16);
					setInsertionState(n71, 2);
					createIf(() => unref(virtualX_on), () => {
						const n70 = t16();
						renderEffect(() => {
							const _virtualX_offsetRight = unref(virtualX_offsetRight);
							setStyle(n70, `min-width:${_virtualX_offsetRight}px;width:${_virtualX_offsetRight}px`);
						});
						return n70;
					});
					return n71;
				}, (footRow, footRowIndex) => footRowIndex, 40);
			}, { _: 8 }));
		});
		renderEffect(() => setStyle(n159, isExperimentalScrollY.value ? `transform:translateY(${unref(virtualScroll).translateY}px)` : ""));
		setInsertionState(n159);
		createIf(() => !isExperimentalScrollY.value && unref(virtual_on) && !unref(isSRBRActive), () => {
			const n91 = t18();
			renderEffect(() => setStyle(n91, paddingTopStyle.value));
			setInsertionState(n91);
			createIf(() => __props.fixedMode && __props.headless, () => {
				return [
					createIf(() => unref(virtualX_on), () => {
						const n79 = t12();
						renderEffect(() => {
							const _theadVirtualX_offsetLeft = unref(theadVirtualX).offsetLeft;
							setStyle(n79, `min-width:${_theadVirtualX_offsetLeft}px;width:${_theadVirtualX_offsetLeft}px`);
						});
						return n79;
					}),
					createFor(() => unref(virtualX_columnPart), (_for_item0, _for_key0) => {
						return createIf(() => _for_item0.value.__VT_C_SP__, () => {
							const n84 = t13();
							renderEffect(() => setProp(n84, "colspan", _for_item0.value.__VT_C_SP__));
							return n84;
						}, () => {
							const n86 = t17();
							renderEffect(() => {
								const _TagType = unref(TagType);
								setStyle(n86, cellStyleMap.value[_TagType.TD].get(colKeyGen.value(_for_item0.value)));
							});
							return n86;
						}, 4618);
					}, (col, _colIdx) => col.__VT_C_SP__ ? `spacer-${_colIdx}` : colKeyGen.value(col), 16),
					createIf(() => unref(virtualX_on), () => {
						...
						renderEffect(() => {
							const _virtualX_offsetRight = unref(virtualX_offsetRight);
							setStyle(n90, `min-width:${_virtualX_offsetRight}px;width:${_virtualX_offsetRight}px`);
						});
						return n90;
					})
				];
		...
		setInsertionState(n159, 1);
		createFor(() => bodyRenderItems.value, (_for_item0) => {
			return createIf(() => _for_item0.value.type === "row", () => {
				const n144 = t10();
				renderEffect(() => {
					const _item = _for_item0.value;
					setDynamicProps(n144, [getTRProps(_item.row, _item.rowIndex)]);
				});
				setInsertionState(n144);
				createIf(() => _for_item0.value.row && _for_item0.value.row.__EXP_R__, () => {
					const n102 = t20();
					const n101 = child(n102);
					renderEffect(() => setProp(n102, "colspan", unref(expandRowColspan)));
					setInsertionState(n101);
					createSlot("expand", {
						row: () => _for_item0.value.row.__EXP_R__,
						col: () => _for_item0.value.row.__EXP_C__
					}, () => {
						const n100 = t19();
						renderEffect(() => {
							const _item_row = _for_item0.value.row;
							setText(n100, toDisplayString(_item_row.__EXP_R__ && _item_row.__EXP_C__ && _item_row.__EXP_R__[_item_row.__EXP_C__.dataIndex] || ""));
						});
						return n100;
					});
					return n102;
					...
					return [
						...
						createFor(() => getBodyColumns(_for_item0.value.row, _for_item0.value.rowIndex), (_for_item1, _for_key1) => {
							return createIf(() => _for_item1.value.__VT_C_SP__, () => {
								const n111 = t13();
								renderEffect(() => setProp(n111, "colspan", _for_item1.value.__VT_C_SP__));
								return n111;
							}, () => createIf(() => _for_item1.value.__VT_PH__, () => {
								const n113 = t21();
								renderEffect(() => setProp(n113, "colspan", _for_item1.value.__VT_PH__));
								return n113;
							}, () => createIf(() => !shouldHideCell(_for_item0.value.row, _for_item1.value), () => {
								const n139 = t17();
								renderEffect(() => {
									const _item = _for_item0.value;
									setDynamicProps(n139, [getTDProps(_item.row, _for_item1.value, _item.rowIndex, _for_item1.value.__LF_S__ ?? 0)]);
								});
								setInsertionState(n139);
								createIf(() => _for_item1.value.customCell, () => {
									return createDynamicComponent(() => _for_item1.value.customCell, {
										class: "table-cell-wrapper",
										tabindex: "-1",
										col: () => _for_item1.value,
										row: () => _for_item0.value.row,
										rowIndex: () => getAbsoluteRowIndex(_for_item0.value.rowIndex),
										colIndex: () => _for_item1.value.__LF_S__ ?? 0,
										cellValue: () => _for_item0.value.row && _for_item0.value.row[_for_item1.value.dataIndex],
										expanded: () => _for_item0.value.row && _for_item0.value.row.__EXP__,
										"tree-expanded": () => _for_item0.value.row && _for_item0.value.row.__T_EXP__
									}, {
										"stkFoldIcon": () => {
											const _on_click = ($event) => triangleClick($event, _for_item0.value.row, _for_item1.value);
											return createComponent(TriangleIcon_default, { onClick: () => _on_click });
										},
										"stkDragIcon": () => {
											const _on_dragstart = ($event) => unref(onTrDragStart)($event, getAbsoluteRowIndex(_for_item0.value.rowIndex));
											return createComponent(DragHandle_default, { onDragstart: () => _on_dragstart });
										}
									});
								}, () => createIf(() => !_for_item1.value.type, () => {
									const n124 = t22();
									const x124 = txt(n124);
									renderEffect(() => {
										const _item = _for_item0.value;
										...
										const _col_dataIndex = _col.dataIndex;
										setProp(n124, "title", _item_row[_col_dataIndex] || "");
										setText(x124, toDisplayString((_item_row && _item_row[_col_dataIndex]) != null ? _item_row && _item_row[_col_dataIndex] : getEmptyCellText.value(_col, _item.row)));
									});
									return n124;
								}, () => createIf(() => _for_item1.value.type === "seq", () => {
									const n126 = t22();
									const x126 = txt(n126);
									renderEffect(() => setText(x126, toDisplayString((props.seqConfig.startIndex || 0) + getAbsoluteRowIndex(_for_item0.value.rowIndex) + 1)));
									return n126;
								}, () => createIf(() => _for_item1.value.type === "tree-node", () => {
									return createComponent(TreeNodeCell_default, {
										class: "table-cell-wrapper",
										tabindex: "-1",
										col: () => _for_item1.value,
										row: () => _for_item0.value.row
									...
									const n138 = t23();
									renderEffect(() => setProp(n138, "title", _for_item0.value.row[_for_item1.value.dataIndex] || ""));
									setInsertionState(n138);
									createIf(() => _for_item1.value.type === "dragRow", () => {
										const _on_dragstart1 = ($event) => unref(onTrDragStart)($event, getAbsoluteRowIndex(_for_item0.value.rowIndex));
										return createComponent(DragHandle_default, { onDragstart: () => _on_dragstart1 });
									}, () => createIf(() => _for_item1.value.type === "expand", () => {
										return createComponent(TriangleIcon_default);
									}), 7941);
									setInsertionState(n138, 1);
									createIf(() => _for_item0.value.row[_for_item1.value.dataIndex] != null, () => {
										const n137 = t14();
										const x137 = txt(n137);
										renderEffect(() => setText(x137, toDisplayString(_for_item0.value.row[_for_item1.value.dataIndex])));
										return n137;
									});
									return n138;
								}, 7685), 7429), 7173), 6917);
						...
						}, (col, _colIdx) => col.__VT_C_SP__ ? `spacer-${_colIdx}` : col.__VT_PH__ ? `ph-${_colIdx}` : colKeyGen.value(col), 16),
						createIf(() => unref(virtualX_on), () => {
							return t16();
						})
					];
				}, 5641);
				_setTemplateRef(n144, trRef, true, "trRef");
				return n144;
			}, () => createIf(() => _for_item0.value.type === "above-ph", () => {
				const n146 = t24();
				renderEffect(() => {
					const _item_count = _for_item0.value.count;
					setAttr(n146, "data-above-count", _item_count);
					setStyle(n146, `height: calc(var(--row-height) * ${_item_count})`);
				});
				return n146;
			}, () => {
				const n149 = t25();
				renderEffect(() => {
					const _item_count = _for_item0.value.count;
					setAttr(n149, "data-below-count", _item_count);
					setStyle(n149, `height: calc(var(--row-height) * ${_item_count})`);
				});
				return n149;
			}, 9226), 8970);
		}, (item) => item.key, 16);
		setInsertionState(n159, 2);
		createIf(() => !isExperimentalScrollY.value, () => {
			return [createIf(() => unref(virtual_on) && !unref(isSRBRActive), () => {
				const n155 = t10();
				renderEffect(() => setStyle(n155, offsetBottomStyle.value));
				return n155;
			}), createIf(() => SRBRBottomHeight.value, () => {
				const n158 = t10();
				renderEffect(() => setStyle(n158, SRBRBottomStyle.value));
				return n158;
			})];
		}, null, 2);
		on(n159, "click", onCellClick);
		on(n159, "mousedown", onCellMouseDown);
		on(n159, "mouseover", onCellMouseOver);
		on(n159, "mouseout", onTbodyMouseOut);
		on(n159, "drop", onBodyDrop);
		on(n161, "dragover", (e) => unref(onTrDragOver)(e));
		on(n161, "dragenter", (e) => unref(onTrDragEnter)(e));
		on(n161, "dragend", (e) => unref(onTrDragEnd)(e));
		on(n161, "click", onRowClick);
		on(n161, "dblclick", onRowDblclick);
		on(n161, "contextmenu", onRowMenu);
		on(n161, "mouseover", onTrMouseOver);
		setInsertionState(n167, 1);
		createIf(() => scrollbarOptions.value.enabled && unref(showScrollbar).y, () => {
			const n166 = t26();
			on(n166, "mousedown", (e) => unref(onVerticalScrollbarMouseDown)(e));
			on(n166, "touchstart", (e) => unref(onVerticalScrollbarMouseDown)(e), { passive: true });
			renderEffect(() => {
				const _scrollbar = unref(scrollbar);
				setStyle(n166, `height:${_scrollbar.h}px;transform:translateY(${_scrollbar.t}px)`);
			});
			return n166;
		});
		setInsertionState(n179, 3);
		createIf(() => (!dataSourceCopy.value || !dataSourceCopy.value.length) && __props.showNoData, () => {
			const n173 = t28();
			renderEffect(() => setClassName(n173, __props.noDataFull ? 1 : 0, " no-data-full", "stk-table-no-data"));
			setInsertionState(n173);
			createSlot("empty", null, () => {
				return t27();
			});
			return n173;
		});
		setInsertionState(n179, 4);
		createSlot("customBottom");
		setInsertionState(n179, 5);
		createIf(() => scrollbarOptions.value.enabled && unref(showScrollbar).x, () => {
			const n177 = t29();
			on(n177, "mousedown", (e) => unref(onHorizontalScrollbarMouseDown)(e));
			on(n177, "touchstart", (e) => unref(onHorizontalScrollbarMouseDown)(e), { passive: true });
			renderEffect(() => {
				const _scrollbar = unref(scrollbar);
				setStyle(n177, `width:${_scrollbar.w}px;transform:translateX(${_scrollbar.l}px)`);
			});
			return n177;
		});
		on(n179, "scroll", onTableScroll);
		on(n179, "wheel", onTableWheel);
		setStaticTemplateRef(n179, tableContainerRef, null, "tableContainerRef");
		return n179;
	}
});
//#endregion
