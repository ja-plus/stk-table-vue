...
if (toTest.size) {
    // lib/ 为被 git 跟踪的构建产物，perf 会在各版本上反复 build 覆盖它；
    // 先丢弃其改动（产物可随时通过 pnpm build 重建），避免跨版本 checkout 被阻塞
    if ((exec('git status --porcelain -- lib') || '').trim()) {
        log('Cleaning tracked lib/ build artifacts (regenerated by pnpm build)...');
        exec('git checkout -- lib');
        exec('git clean -fd -- lib');
    }

    // 其余未提交改动会污染测试结果或阻塞切换分支，直接拒绝运行
