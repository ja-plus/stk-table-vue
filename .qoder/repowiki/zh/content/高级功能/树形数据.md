...
- [docs-demo/basic/tree/TreeDefaultExpandKeys.vue](file://docs-demo/basic/tree/TreeDefaultExpandKeys.vue)
...
## 更新摘要
**变更内容**   
- 增强了 setTreeExpand 和 setRowExpand 方法，支持字符串和数字类型的行键
- 新增演示组件 TreeDefaultExpandKeys.vue，展示通过 setTreeExpand() 方法进行程序化控制树节点展开
- 更新了文档说明，明确了 treeConfig.defaultExpandedKeys 在异步数据场景下的限制
...
**更新** 增强了 setTreeExpand 和 privateSetTreeExpand 方法，现在支持字符串和数字类型的行键参数，提高了 API 的灵活性和易用性。
...
**更新** setTreeExpand 和 setRowExpand 方法现在都支持字符串和数字类型的行键，提供了更灵活的编程式控制能力。
...
- **新增** TreeDefaultExpandKeys.vue：展示通过 setTreeExpand() 方法进行程序化控制的示例
- config.ts：默认展开策略、键名映射等配置

**更新** 新增了 TreeDefaultExpandKeys.vue 演示组件，展示了如何使用 setTreeExpand(['China']) 来程序化控制特定节点的展开状态。
...
**更新** 由于 setTreeExpand 和 setRowExpand 方法现在支持多种类型的行键，在处理大量数据时需要特别注意行键的唯一性和类型一致性，以避免性能问题。
...
- 常见问题
  ...
  - **新增** 行键类型不匹配：确保传入的行键类型与数据中的实际类型一致（字符串或数字）
- 调试技巧
  - 打印 useTree 的内部状态（展开集合、缓存、可见行索引）
  - 使用浏览器性能面板分析渲染耗时
  - 逐步缩小数据规模定位问题
- **新增** 异步数据场景注意事项
  - treeConfig.defaultExpandedKeys 仅在初次渲染时生效
  - 对于异步加载的数据，应使用 setTreeExpand() 方法进行程序化控制
...
## 结论
stk-table-vue 的树形数据功能通过 useTree.ts 与 TreeNodeCell.vue 的紧密协作，实现了高性能、可扩展的树形表格体验。结合虚拟滚动与懒加载，能够有效应对大数据量场景。最新的增强使得 setTreeExpand 和 setRowExpand 方法更加灵活，支持字符串和数字类型的行键，为开发者提供了更好的编程式控制能力。开发者可根据实际需求选择合适的配置与优化策略，以获得最佳的用户体验。
...
**更新** 新增了 TreeDefaultExpandKeys.vue 演示，展示了如何使用增强的 setTreeExpand() 方法进行程序化控制。
