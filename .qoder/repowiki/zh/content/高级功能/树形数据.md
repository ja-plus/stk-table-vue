...
- 操作方法（展开/折叠、搜索过滤等）
- **新增**：程序化控制树节点展开状态的 `setTreeExpand()` API 方法
- 大数据量场景下的性能优化建议与实践
...
- 主表入口：StkTable.vue（启用树模式、与虚拟滚动协同、暴露 setTreeExpand API）
...
A --> H["expose.setTreeExpand()<br/>程序化控制API"]
```
...
- StkTable.vue：作为表格容器，启用树模式后：
  ...
  - **新增**：通过 `defineExpose` 暴露 `setTreeExpand` API 方法
...
V-->>T : 重绘可见行
Note over T : 程序化控制
U->>T : 调用 setTreeExpand()
T->>Tree : privateSetTreeExpand()
Tree->>Tree : 更新展开状态
Tree->>V : 刷新虚拟滚动
```
...
- **新增**：`setTreeExpand()` API 方法
  - 支持通过 rowKey 或 row 对象进行程序化控制
  - 支持单个节点或多个节点的批量操作
  - 可选的 expand 参数控制展开或收起状态
  - 不传参数时自动反转当前状态
- 懒加载策略
...
UserAction --> |点击| Toggle["toggleTreeNode()<br/>用户交互"]
UserAction --> |API调用| SetExpand["setTreeExpand()<br/>程序化控制"]
Toggle --> PrivateExpand["privateSetTreeExpand()<br/>isClick: true"]
SetExpand --> PrivateExpand
PrivateExpand --> CheckLoad["检查是否已加载子节点"]
CheckLoad --> |未加载| Load["触发懒加载回调"]
Load --> Cache["缓存子节点结果"]
Cache --> Update["更新展开集合与可见行"]
CheckLoad --> |已加载| Update
Update --> EmitEvent["触发 toggle-tree-expand 事件"]
EmitEvent --> Render
```
...
- **新增**：`setTreeExpand()` API 暴露
  - 通过 `defineExpose` 方法暴露给外部调用
  - 支持在组件外部程序化控制树节点展开状态
  - 与内部 `toggleTreeNode` 方法形成互补
...
Tree->>Virtual : 刷新可见范围
Virtual-->>Table : 重绘可见行
Note over Table : 外部API调用
External->>Table : setTreeExpand(row, option)
Table->>Tree : privateSetTreeExpand()
Tree->>Virtual : 更新虚拟滚动
```
...
- **新增**：TreeDefaultExpandKeys.vue：展示如何使用 `setTreeExpand()` 方法进行程序化控制
...
- 交互优化
  - 展开/收起操作防抖，减少频繁状态更新
  - 批量操作合并更新，降低重绘次数
  - **新增**：`setTreeExpand()` 方法支持批量操作，提高性能
- 内存管理
  - 及时清理未使用的缓存与监听器
  - 避免闭包持有大量数据引用
...
- 常见问题
  ...
  - **新增**：`setTreeExpand()` 调用无效：检查 rowKey 是否正确，节点是否存在
- 调试技巧
  - 打印 useTree 的内部状态（展开集合、缓存、可见行索引）
  - 使用浏览器性能面板分析渲染耗时
  - 逐步缩小数据规模定位问题
  - **新增**：在调用 `setTreeExpand()` 前后检查树状态变化
...
## 结论
stk-table-vue 的树形数据功能通过 useTree.ts 与 TreeNodeCell.vue 的紧密协作，实现了高性能、可扩展的树形表格体验。**新增的 `setTreeExpand()` API 方法**为开发者提供了强大的程序化控制能力，可以灵活地管理树节点的展开状态。结合虚拟滚动与懒加载，能够有效应对大数据量场景。开发者可根据实际需求选择合适的配置与优化策略，以获得最佳的用户体验。

## 附录
- API 参考：docs-src/main/table/basic/tree.md、docs-src/main/api/expose.md
- 演示代码：docs-demo/basic/tree/*
- 独立虚拟树：src/VirtualTree.vue
- **新增**：`setTreeExpand()` API 使用方法示例
