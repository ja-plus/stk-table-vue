...
### 1. 安装
支持多种包管理器，任选其一执行：
- npm: npm install stk-table-vue
- yarn: yarn add stk-table-vue
- pnpm: pnpm add stk-table-vue
...
## 结论
通过以上步骤，你可以在几分钟内完成 stk-table-vue 的安装、配置与首个表格的运行。建议在掌握基础用法后，逐步探索排序、筛选、虚拟滚动、自定义单元格等高级特性，以满足复杂业务场景。

[本节为总结性内容，不直接分析具体文件]

## 附录：从零到运行示例
- 创建项目：使用 Vite 创建 Vue 项目
- 安装依赖：npm/yarn/pnpm 安装 stk-table-vue
